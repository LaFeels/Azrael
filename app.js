...
const XP_BASE = 80;
const XP_EXPONENT = 1.18;
const SKILL_PTS_PER_RANK = 60;
...
function soulPower(pts) { return Math.min(Math.floor((pts / 300) * 100), 100); }
...
function renderSlider(edict) {
  ...
  return `
    <div class="slider-section">
      <div class="slider-progress-row">
        <div>
          <span class="slider-current ${atGoal?'at-goal':''}">${progress}</span>
          <span class="slider-unit">${edict.unit}</span>
        </div>
        <span class="slider-target">of ${edict.target}${edict.unit}</span>
      </div>
      <div class="slider-wrap">
        <div class="slider-track-bg">
          <div class="slider-track-fill ${atGoal?'fill-gold':pl.fillClass}" style="width:${pct}%"></div>
        </div>
        <div class="slider-thumb ${atGoal?'at-goal':''}" style="left:${Math.min(pct,98)}%">
          <div class="slider-dot ${atGoal?'at-goal':''}"></div>
        </div>
        <input type="range" class="slider-input" min="0" max="${edict.target}" step="1" value="${progress}" data-edict-id="${edict.id}" ${state.dailyDone.includes(edict.id)?'disabled':''}>
      </div>
      ${ticks}
      <div class="partial-earned" style="display:${!atGoal&&progress>0?'block':'none'}">
        Progress earns <span>+${earned} SF</span>
      </div>
    </div>
  `;
}
...
function renderEdictCard(edict) {
  ...
  const soulNote = edict.requiresNote && done ? `
    <div class="soul-note-label">What was tended to</div>
    <textarea class="soul-note-input" placeholder="What did the soul receive today..." rows="3" data-soul-note="${edict.id}"></textarea>
  ` : '';

  const missExplain = isMissed && edict.isCore && !state.missedCore.find(m => m.date === new Date().toDateString() && m.edictId === edict.id) ? `
    <div class="miss-explain">
      <div class="miss-label">Explain the absence. One honest sentence.</div>
      <input class="miss-input" placeholder="Why it didn't happen..." data-miss-id="${edict.id}">
      <button class="miss-submit" onclick="submitMiss('${edict.id}')">Record & Close</button>
    </div>
  ` : '';

  const checkbox = edict.type === 'checkbox' ? `
    <div class="edict-checkbox ${done?'checked':''}" onclick="handleEdictCheck('${edict.id}')">
      ${done ? '<span class="check-mark">✦</span>' : isMissed ? '<span class="miss-mark">✕</span>' : ''}
    </div>
  ` : '';
  ...
  return `
    <div class="edict-card ${edict.isCore?'is-core':''} ${atGoal?'is-done':''} ${isMissed?'is-missed':''}" data-edict-id="${edict.id}">
      <div class="edict-stripe ${pl.stripeClass}"></div>
      ${edict.isCore ? '<div class="core-badge">◈ Core</div>' : '<div class="stretch-badge">✦ Stretch</div>'}
      <div class="edict-inner">
        <div class="edict-top">
          ${checkbox}
          <div class="edict-main">
            <div class="edict-label ${atGoal?'done-text':''}">${edict.label}</div>
            <div class="edict-meta">
              ${renderChip(edict.skill)}
              ${renderTypeBadge(edict)}
              <span class="edict-xp-tag">${atGoal?'✦ ':'+'} ${edict.xp} SF</span>
            </div>
          </div>
        </div>
        ${slider}
  ...
  const bars = data.map((d,i) => {
    ...
    return `
      <g>
        ...
        <text x="${x+bw/2}" y="${H+14}" fill="#8a6838" font-size="7" text-anchor="middle" font-family="Cinzel">${d.label}</text>
        ${d.total>0?`<text x="${x+bw/2}" y="${y-4}" fill="#8a5f18" font-size="7" text-anchor="middle" font-family="Cinzel">${Math.round(d.total)}</text>`:''}
      </g>
    `;
  }).join('');
    ...
    <svg viewBox="0 0 ${W} ${H+26}" style="width:100%;overflow:visible">
      <defs>
        <linearGradient id="barGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#b07828" stop-opacity="0.9"/>
          <stop offset="100%" stop-color="#6a4010" stop-opacity="0.3"/>
        </linearGradient>
      </defs>
      ${bars}
    </svg>
...
/* ── MAIN RENDER ── */
function render() {
  const { level, currentXP, toNext } = getLevel(state.xp);
  const xpPct = (currentXP / toNext) * 100;
  ...
  document.getElementById('tab-status').innerHTML = `
    <div class="hero">
      ...
      <div class="player-class">Fallen Angel · ${RANKS[Math.min(level-1,RANKS.length-1)]}</div>
      <div class="level-block">
        <div class="level-num">${level}</div>
        <div class="level-meta">
          <div class="level-tag">Ascension Level</div>
          <div class="level-title">${RANKS[Math.min(level-1,RANKS.length-1)]}</div>
          <div class="level-sub">${currentXP} / ${toNext} SF to ascend</div>
        </div>
      </div>
      <div class="xp-wrap">
        <div class="xp-head"><span>Soul Fragments</span><span>${currentXP} / ${toNext}</span></div>
  ...
  document.getElementById('edict-sub-active').innerHTML = `
    <div class="quest-fp" style="margin-bottom:13px">
      <div class="quest-eyebrow">Daily Quest</div>
      <div class="quest-text">${quest.text}</div>
      <div class="quest-foot">
        <div>
          <div class="quest-xp">+${quest.xp} SF</div>
          <div class="quest-skill">${PILLARS[quest.skill].name}</div>
        </div>
        ${state.questDone ? '<div class="quest-done">✦ Complete</div>' : '<button class="btn btn-burg" onclick="completeQuest()">Complete</button>'}
      </div>
    </div>
    <div class="day-sep"><div class="day-sep-line"></div><div class="day-sep-label">◈ Core Edicts</div><div class="day-sep-line"></div></div>
    ${coreHTML}
    ${activeLib.length > 0 ? `
      <div class="day-sep"><div class="day-sep-line"></div><div class="day-sep-label">Personal Edicts</div><div class="day-sep-line"></div></div>
      ${libActiveHTML}
    ` : ''}
    <div class="day-sep"><div class="day-sep-line"></div><div class="day-sep-label">✦ Stretch Goals</div><div class="day-sep-line"></div></div>
    <div style="font-size:12px;color:var(--t3);font-style:italic;margin-bottom:12px">Rotate every day. Escalate as you ascend.</div>
    ${stretchHTML}
    <button class="btn btn-warm btn-full mt-8" onclick="openModal('inscribe')">✦ Add to Library</button>
  `;

  document.getElementById('edict-sub-library').innerHTML = `
    <div style="font-size:13px;color:var(--t3);font-style:italic;margin-bottom:14px;line-height:1.5">Every edict inscribed. Toggle to add to your daily list.</div>
    ${state.edictLibrary.length === 0 ? '<div class="empty">The library is empty. Inscribe your first edict.</div>' : ''}
    ${state.edictLibrary.map(e => {
      const pl = PILLARS[e.skill];
      const isActive = state.activeLibraryIds.includes(e.id);
      return `
        <div class="lib-item">
    ...
    <button class="btn btn-warm btn-full mt-8" onclick="openModal('inscribe')">✦ Inscribe New Edict</button>
  `;

  document.getElementById('edict-sub-missed').innerHTML = `
    <div style="font-size:13px;color:var(--t3);font-style:italic;margin-bottom:14px;line-height:1.5">A permanent record. These do not disappear.</div>
    ${state.missedCore.length === 0 ? '<div class="empty">No core edicts missed. The record is clean.</div>' : ''}
    ${[...state.missedCore].reverse().map(m => `
  ...
  // ── CODEX TAB ──
  document.getElementById('tab-codex').innerHTML = `
    <div class="sh"><div class="sh-bar"></div><div class="sh-title">The Codex</div><div class="sh-line"></div></div>
    <div style="font-size:14px;color:var(--t3);font-style:italic;margin-bottom:16px;line-height:1.5">Five disciplines. Each grows independently and feeds your ascension. Soul carries the most weight.</div>
    ${Object.entries(PILLARS).map(([key,pl]) => {
      const pts = state.skills[key] || 0;
      const r = skillRank(pts), p = skillPct(pts);
      const totalFocus = state.focusLog.filter(l=>l.skill===key).reduce((a,l)=>a+l.minutes,0);
      return `
        <div class="skill-panel">
          ...
          <div class="skill-inner">
            ...
            ${totalFocus > 0 ? `<div style="margin-top:6px;font-family:'Crimson Pro',serif;font-size:11px;font-style:italic;color:var(--t3)">${formatMins(totalFocus)} logged lifetime</div>` : ''}
  ...
  document.getElementById('tab-focus').innerHTML = `
    <div class="sh"><div class="sh-bar"></div><div class="sh-title">Focus Session</div><div class="sh-line"></div></div>
    ${!isTimerActive ? `
      ...
      ${Object.entries(PILLARS).map(([key,pl]) => {
        ...
        return `
          <button class="focus-btn ${currentFocusPillar===key?'active':''}" onclick="setFocusPillar('${key}')">
            <span class="focus-icon">${pl.icon}</span>
            <div>
              <div class="focus-name">${pl.name}</div>
              <div class="focus-today">Today: ${formatMins(todayMins)||'—'}</div>
            </div>
            ${currentFocusPillar===key?'<span class="focus-mark">✦</span>':''}
          </button>
        `;
      }).join('')}
    ...
    <div class="graph-wrap">
      <div class="graph-title">✦ Focus Across Time</div>
      <div class="graph-tabs">
        ${['week','month','year'].map(v=>`<button class="graph-tab ${graphView===v?'active':''}" onclick="setGraphView('${v}')">${v.charAt(0).toUpperCase()+v.slice(1)}</button>`).join('')}
      </div>
      ${state.focusLog.length === 0 ? '<div class="empty">No sessions yet</div>' : renderGraph()}
    </div>
    <div class="sh" style="margin-top:14px"><div class="sh-bar"></div><div class="sh-title">All Time</div><div class="sh-line"></div></div>
    ${Object.entries(PILLARS).map(([key,pl]) => {
      const total = state.focusLog.filter(l=>l.skill===key).reduce((a,l)=>a+l.minutes,0);
      return `
  ...
  // ── SOUL TAB ──
  document.getElementById('tab-soul').innerHTML = `
    <div class="fp" style="background:linear-gradient(158deg,#f5eef8,#ede5f0);border:1px solid rgba(90,56,168,.2);margin-bottom:13px">
      <div class="orn tl"></div><div class="orn tr"></div><div class="orn bl"></div><div class="orn br"></div>
      <div class="fp-header" style="border-bottom-color:rgba(90,56,168,.15)">
        <div class="fp-bar" style="background:linear-gradient(180deg,var(--s3),var(--s2))"></div>
        <div class="fp-title" style="color:var(--s3)">The Soul</div>
        <div class="fp-line"></div>
        <div class="fp-tag">Rank ${soulRank}</div>
      </div>
      <div class="fp-body">
        <div style="font-family:'Crimson Pro',serif;font-size:15px;font-style:italic;color:var(--t1);line-height:1.7;margin-bottom:14px">
          "All powers flow from here. The strength, the foresight, the stillness. While others upgrade their egos, Azrael dims his. The soul is the source of everything."
        </div>
        <div class="soul-indicator">
          <div class="soul-orb"></div>
          <div style="flex:1"><div class="soul-title">Ego Dissolution</div><div class="soul-sub">The quieter the ego, the louder the soul</div></div>
          <div class="soul-pct">${soulPower(soulPts)}%</div>
        </div>
      </div>
    </div>
    <div class="sh"><div class="sh-bar" style="background:linear-gradient(180deg,var(--s3),var(--s2))"></div><div class="sh-title" style="color:var(--s3)">Today's Chronicle</div><div class="sh-line"></div></div>
    <div class="reflect-panel">
      <div class="orn tl"></div><div class="orn tr"></div>
      <div class="reflect-q-label">Today's Reflection</div>
      <div class="reflect-q">${rp}</div>
      ${state.todayMood !== null ? `<div style="font-size:24px;margin-bottom:8px">${MOODS[state.todayMood]}</div>` : ''}
      ${state.todayReflect
        ? `<div style="font-family:'Crimson Pro',serif;font-size:14px;color:var(--t2);font-style:italic;line-height:1.6;margin-bottom:12px">${state.todayReflect}</div>`
        : '<div style="font-size:13px;color:var(--t3);font-style:italic;margin-bottom:12px">Not yet answered today.</div>'
      }
      <button class="btn btn-soul btn-full" onclick="openModal('journal')">${state.todayReflect?'Update Entry':'Open Chronicle'} ✦</button>
    </div>
    <div class="sh" style="margin-top:16px"><div class="sh-bar"></div><div class="sh-title">Past Entries</div><div class="sh-line"></div></div>
    ${state.journalEntries.length === 0 ? '<div class="empty">The chronicle awaits its first entry.</div>' : ''}
    ${state.journalEntries.slice(0,10).map(entry => `
      <div class="fp" style="margin-bottom:8px">
  ...
  document.getElementById('tab-goals').innerHTML = `
    <div class="sh"><div class="sh-bar"></div><div class="sh-title">Covenants</div><div class="sh-line"></div></div>
    ...
    ${[
      { key:'day100', icon:'🔥', name:'Day 100', desc:'The first real milestone', check:() => daysRunning >= 100, date:state.milestones?.day100 },
      { key:'firstRank5', icon:'⚡', name:'First Rank 5', desc: state.milestones?.firstRank5 ? PILLARS[state.milestones.firstRank5.pillar]?.name : 'Any pillar reaches Rank 5', check:() => !!state.milestones?.firstRank5, date:state.milestones?.firstRank5?.date },
      { key:'firstCovenant', icon:'✦', name:'First Covenant Fulfilled', desc: state.milestones?.firstCovenant?.name || 'Complete your first goal', check:() => !!state.milestones?.firstCovenant, date:state.milestones?.firstCovenant?.date },
    ].map(m => {
  ...
  document.getElementById('tab-treasury').innerHTML = `
    <div class="vault-panel">
      <div class="orn tl" style="border-color:rgba(36,88,40,.4)"></div><div class="orn tr" style="border-color:rgba(36,88,40,.4)"></div>
      <div class="vault-label">Vault — Savings</div>
      <div class="vault-amount">$${state.savings.toFixed(2)}</div>
      ${state.savingsGoal > 0 ? `
