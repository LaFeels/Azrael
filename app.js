...
+’<div class="quest-foot"><div><div class="quest-xp">+’+Math.max(15, 20 + (quest.mins || 0))+’ SF</div><div class="quest-skill">’+PILLARS[quest.skill].name+’</div></div>’
...
function questXP(q) { return Math.max(15, 20 + (q.mins || 0)); }
function completeQuest() {
if(state.questDone) return;
var q=getDailyQuest(), xp=questXP(q);
...
var quest=getDailyQuest(), questXP=Math.max(15, 20 + (quest.mins || 0)), html=’’;
...
html+=’<div class="sh" style="margin-top:14px"><div class="sh-bar"></div><div class="fp-title" style="font-family:\'Cinzel\',serif;font-size:9px;font-weight:700;letter-spacing:3px;color:var(--g3);text-transform:uppercase">Time By Discipline</div><div class="sh-line"></div></div>’;
var allPillars2=[‘comp’,‘labor’,‘vigil’,‘soul’], pi2, k2, pl2, pillarTotal, fi2;
for(pi2=0;pi2<allPillars2.length;pi2++){
k2=allPillars2[pi2]; pl2=PILLARS[k2]; pillarTotal=0;
for(fi2=0;fi2<state.focusLog.length;fi2++) if(state.focusLog[fi2].skill===k2) pillarTotal+=state.focusLog[fi2].minutes;
html+=’<div class="fp" style="margin-bottom:8px"><div class="fp-body" style="padding:11px 14px">’
+’<div style="display:flex;align-items:center;gap:12px">’
+’<span style="font-size:20px">’+pl2.icon+’</span>’
+’<div style="flex:1"><div style="font-family:\'Cinzel\',serif;font-size:11px;color:var(--t1);letter-spacing:1px;font-weight:600">’+pl2.name+’</div>’
+’<div style="font-size:12px;color:var(--t3);font-style:italic;margin-top:2px">’+(pillarTotal>0?formatMins(pillarTotal)+’ focused’:‘No sessions yet’)+’</div></div>’
+’<div style="font-family:\'Cinzel\',serif;font-size:18px;font-weight:900;color:var(--g2)">’+(pillarTotal>0?formatMins(pillarTotal):’—’)+’</div>’
+’</div></div></div>’;
}
...
var jslice=state.journalEntries.slice(0,30);
for(i=0;i<jslice.length;i++){entry=jslice[i];
html+=’<div class="fp" style="margin-bottom:8px;cursor:pointer" onclick="openEntry('+i+')">’
+’<div class="fp-body" style="padding:12px 14px">’
+’<div class="flex-between" style="margin-bottom:5px">’
+’<div style="font-family:\'Cinzel\',serif;font-size:9px;letter-spacing:1px;color:var(--t3)">’+esc(entry.date)+’</div>’
+(entry.mood!=null?’<div style="font-size:16px">’+MOODS[entry.mood]+’</div>’:’’)
+’</div>’
+(entry.journal?’<div style="font-family:\'Crimson Pro\',serif;font-size:13px;color:var(--t2);line-height:1.5;margin-bottom:4px">’+esc(entry.journal.slice(0,80))+(entry.journal.length>80?’…’:’’)+’</div>’:’’)
+’<div style="font-family:\'Cinzel\',serif;font-size:7px;letter-spacing:1px;color:var(--g2);margin-top:4px">Tap to read full entry ›</div>’
+’</div></div>’;
}
...
window.openEntry=function(i){
var entry=state.journalEntries[i]; if(!entry) return;
var modal=document.getElementById(‘modal-view-entry’);
if(!modal) return;
var html=’<div style="font-family:\'Cinzel\',serif;font-size:8px;letter-spacing:2px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">’+esc(entry.date)+’</div>’;
if(entry.mood!=null) html+=’<div style="font-size:24px;margin-bottom:12px">’+MOODS[entry.mood]+’</div>’;
if(entry.journal) html+=’<div style="font-family:\'Cinzel\',serif;font-size:8px;letter-spacing:2px;color:var(--t3);text-transform:uppercase;margin-bottom:6px">Journal</div><div style="font-family:\'Crimson Pro\',serif;font-size:15px;color:var(--t1);line-height:1.7;margin-bottom:16px;white-space:pre-wrap">’+esc(entry.journal)+’</div>’;
if(entry.reflect) html+=’<div style="font-family:\'Cinzel\',serif;font-size:8px;letter-spacing:2px;color:var(--s3);text-transform:uppercase;margin-bottom:6px">◈ Reflection</div><div style="font-family:\'Crimson Pro\',serif;font-size:15px;font-style:italic;color:var(--t1);line-height:1.7;white-space:pre-wrap">’+esc(entry.reflect)+’</div>’;
document.getElementById(‘entry-view-content’).innerHTML=html;
...
+’<div id="modal-view-entry" class="modal-content" style="display:none"><div class="modal-title">✦ Chronicle Entry</div><div id="entry-view-content" style="max-height:60vh;overflow-y:auto"></div><div class="btn-row"><button class="btn btn-ghost btn-full" onclick="closeModal()">Close</button></div></div>’
+’</div>’;
